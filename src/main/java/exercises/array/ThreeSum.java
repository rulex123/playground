 ...
 * the array which gives the sum of zero. Note: The solution set must not contain duplicate triplets.
 *
 * @author emanno
		...
		for ( int i = 0; i < nums.length - 1; i++ ) {
			for ( int j = i + 1; j < nums.length; j++ ) {
				...
				if ( numToIndexMap.containsKey( complement ) ) {
					if ( (complement == nums[ i ] || complement == nums[ j ]) && numToIndexMap.get( complement ).size() == 1 ) // do
																																																											// not
																																																											// reuse
																																																											// nums[i]
																																																											// and
